...
1. 优先双击 `dist\BugStatisticsClient_v18_30.exe`；也可以双击 `run_client.bat` 从源码启动。窗口标题栏会显示当前版本号 `v18.30`。
2. 输入用户名、绩效系统密码和 CRM 密码，点击“登录”。
   两个密码框后的眼睛按钮可分别显示或隐藏密码。
3. 登录区下方通过深色分段导航切换“绩效缺陷明细”、“CRM 缺陷查询”和“工作量录入”；界面采用深蓝科技面板主题，统一使用高亮蓝、青色和状态色。
...
14. “工作量录入”页签支持选择计划版本、后端/前端分组、责任人范围和 `.xlsx` 文件。`开发一组` 等无后缀分组为后端，`开发一组-前端` 等分组为前端。点击“加载并预览”后，会动态读取当前分组对应的工作量表定义、需求列表、开发资源和已有任务；表格中的 Excel 工时和计算工时均按天展示（1 天 = 8 小时）。
15. 预览结果会区分“可提交、提醒、待填写、错误、已存在”。新增空白行先以浅蓝色“待填写”显示，不会在用户尚未录入时整行标红；填写完整后才执行错误、重复和工时校验。“已存在”、“错误”和“待填写”均不能进入提交范围。
16. “工作量录入”是登录后的白名单功能，仅绩效账号 `T0423` 和 `T0101` 可见；其他账号登录后不会显示该页签，也不能进入预览或提交流程。
17. “选择文件”右侧提供“下载模板”，可把内置的标准 `.xlsx` 模板保存到指定位置；模板包含空白导入页和字段填写说明页，下载后会自动成为当前选中的导入文件。导入页预留 100 行带浅灰边框的录入区域，便于在 WPS/Excel 中逐行填写。
18. 预览表格中“可提交”和“提醒”记录默认勾选；选择列表头使用与行内相同大小的复选框，支持全选、半选和取消全选，且只影响可提交记录。“已存在”和“错误”的选择框禁用并显示 Windows 禁止光标。日期通过日历控件选择，双击原有行的“工作描述、Excel 工时”可直接修改并重新校验；每行提供带手形光标的红色删除按钮。
19. “＋ 添加一行”位于“下载模板”后面，并采用白底蓝边的次级按钮样式。预览完成后点击按钮会立即在表格中插入一条浅蓝色空白“新增”行，不显示弹窗；责任人、需求编号、工作描述、计划开始、计划完成和 Excel 工时的空白单元格都会直接显示“待填写”，进入编辑时则使用真正的空输入框。责任人通过下拉框选择，需求编号和工作描述直接手填，计划日期通过日历填写。当前编辑框使用统一白底蓝边样式，表格不再显示深蓝整行选中效果。校验信息在表格中显示短提示，点击状态或校验信息可在底部查看完整原因。
20. 顶部右侧提供深色/浅色主题切换按钮，主题选择会自动保存并在下次启动时沿用。
...
选择“全部组员”时，只会读取当前录入分组成员对应的 Excel 行；其他分组人员会被跳过。预览会同时展示 Excel 工时和规则计算工时，二者不一致时以“提醒”标出。

读取 Excel 时会先验证完整的 xlsx ZIP 结构；如果正好遇到 WPS/Excel 后台保存产生的短暂不完整快照，最长会等待约 15 秒。加密工作簿和旧版 `.xls` 容器会显示独立提示，不再与保存未完成混为同一种错误。
...
前端分组基于 2026 年 1~7 月、8 个前端组约 1983 条历史记录的核心贡献比例生成工作量组合。工作描述会调整主要控件、成熟/新接口和自测权重；只有达到 2 天的任务才允许分配预研工时，其中普通任务约有 50% 自动分配，预研不超过 4 小时，同一任务重复预览结果保持一致；低于 2 天时预研固定为 0，小数尾数计入自测。明确命中关键词时仍会启用局部刷新、通用组件或不成熟组件。算法保证 2h、19.2h 等输入与计算工时精确相等。
