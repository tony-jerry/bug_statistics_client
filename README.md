...
1. 优先双击 `dist\BugStatisticsClient_v18_23.exe`；也可以双击 `run_client.bat` 从源码启动。窗口标题栏会显示当前版本号 `v18.23`。
...
14. “工作量录入”页签支持选择计划版本、后端/前端分组、责任人范围和 `.xlsx` 文件。`开发一组` 等无后缀分组为后端，`开发一组-前端` 等分组为前端。点击“加载并预览”后，会动态读取当前分组对应的工作量表定义、需求列表、开发资源和已有任务；表格中的 Excel 工时和计算工时均明确标注单位为小时。
...
前端分组基于 2026 年 1~7 月、8 个前端组约 1983 条历史记录的核心贡献比例生成工作量组合。工作描述会调整主要控件、成熟/新接口和自测权重；只有明确命中关键词时才启用预研、局部刷新、通用组件或不成熟组件。算法用预研字段承接小于 0.5 小时的尾差，保证 2h、19.2h 等输入与计算工时精确相等，不再把大量余量堆入自测场景。
